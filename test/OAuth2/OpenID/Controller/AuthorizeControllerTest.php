...
class AuthorizeControllerTest extends \PHPUnit_Framework_TestCase
{
    public function testValidateAuthorizeRequest()
    {
        ...
        $this->assertEquals($query['state'], 'xyz');
    ...
    private function getTestServer($config = array())
    {
        ...
        $id_token = new IdToken($storage, $storage, array('issuer' => 'anzev'));
        $access_token = new AccessToken($storage);

        // Override default controller with OpenID
        $authorizeController = new AuthorizeController(
                                $storage,
                                array(
                                    'id_token' => $id_token,
                                    'token id_token' => new TokenIdToken(
                                        $access_token, $id_token
                                    )
                                ), array('allow_implicit' => true));

        $server->setAuthorizeController($authorizeController);
